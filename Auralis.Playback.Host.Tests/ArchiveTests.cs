...
internal static class ArchiveTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "Auralis-playback-archive-" + Guid.NewGuid().ToString("N"));
        ...
        var store = new PlaybackInstallationStore(install, PlaybackCapabilities.Audio);
        var zipPath = Path.Combine(root, "test.auralis-playback.zip");
        var payload = new[] { new Entry("Fixture.dll", [1, 2, 3]), new Entry("native/codec.dll", [4, 5, 6]) };
        var manifest = new JsonObject
        {
            ["schemaVersion"] = 1, ["kind"] = "playback", ["id"] = "fixture.archive", ["displayName"] = "Synthetic",
            ...
            ["capabilities"] = new JsonArray("audio"), ["files"] = new JsonArray(payload.Select(e => (JsonNode)new JsonObject
                { ["path"] = e.Path, ["length"] = e.Bytes.Length, ["sha256"] = Convert.ToHexString(SHA256.HashData(e.Bytes)) }).ToArray())
        };
        Entry[] Entries() => [new(PlaybackComponentManifest.FileName, Encoding.UTF8.GetBytes(manifest.ToJsonString())), .. payload];
        ...
        async Task BadArchive()
        {
            ...
            catch (PlaybackArchiveException e) { Check(!e.ToString().Contains(root), "typed path-free archive failure"); }
        ...
        try
        {
            ...
            Check(store.ReadState().Components.Count == 0 && !File.Exists(Path.Combine(install, "playback-installations.json")), "preview does not approve");
            try { await store.PreviewArchiveAsync(zipPath); throw new Exception("concurrent preview"); }
            catch (PlaybackInstallationException e) { Check(e.Issue == PlaybackInstallationIssue.Busy, "one pending preview per store"); }
            try { await store.ImportApprovedAsync(preview, new(preview.Descriptor.Id, new string('0', 64))); throw new Exception("wrong grant"); }
            catch (PlaybackInstallationException e) { Check(e.Issue == PlaybackInstallationIssue.ApprovalRequired, "archive exact approval"); }
            try { await new PlaybackInstallationStore(install, PlaybackCapabilities.Audio).ImportApprovedAsync(preview, new(preview.Descriptor.Id, preview.ManifestSha256)); throw new Exception("foreign preview"); }
            catch (PlaybackInstallationException e) { Check(e.Issue == PlaybackInstallationIssue.ApprovalRequired, "archive preview owner"); }
            ...
            await using (var tampered = await store.PreviewArchiveAsync(zipPath))
            {
                ...
                catch (PlaybackInstallationException e) { Check(e.Issue == PlaybackInstallationIssue.InvalidPackage, "extracted preview revalidated before install"); }
            }
            ...
            foreach (var rows in new Entry[][]
            {
                ...
                [Entries()[0] with { Path = "Playback.component.json" }, .. payload],
                [.. Entries(), new("empty/", [])],
                ...
                [new(PlaybackComponentManifest.FileName, Encoding.UTF8.GetBytes("{}")), .. payload],
            ...
            Zip([new(PlaybackComponentManifest.FileName, Encoding.UTF8.GetBytes(deepManifest.ToJsonString())), .. deepPayload]);
            ...
            foreach (var mutate in new Action<byte[], int, int>[]
            {
                ...
                (b,c,e) => BinaryPrimitives.WriteUInt32LittleEndian(b.AsSpan(c + 24), 513U * 1024 * 1024),
            ...
            var stateBefore = File.ReadAllText(Path.Combine(install, "playback-installations.json"));
            ...
            catch (PlaybackInstallationException e) { Check(e.Issue == PlaybackInstallationIssue.InvalidPackage, "ZIP compatibility via existing catalog"); }
            Check(PreviewsEmpty() && File.ReadAllText(Path.Combine(install, "playback-installations.json")) == stateBefore, "incompatible archive leaves installed state untouched");
            ...
            Console.WriteLine($"PASS playback archives: {assertions} assertions; bounded ZIP metadata/extraction, path/link/duplicate/CRC-independent payload integrity, exact approval, cancel/dispose, update and inert install.");
        }
        finally
        {
            if (Path.GetDirectoryName(root) == Path.TrimEndingDirectorySeparator(Path.GetTempPath()) &&
                Path.GetFileName(root).StartsWith("Auralis-playback-archive-", StringComparison.Ordinal) &&
