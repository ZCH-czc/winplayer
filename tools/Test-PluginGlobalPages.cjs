...
(async()=>{
  ...
  try{for(const theme of ['light','dark'])for(const scale of [1,1.5,2]){
    ...
    const provider={id:'independent.space',name:'Independent studio',configured:true,pageRevision:1,capabilities:['Pages','GlobalPages'],
    ...
    const makeDoc=m=>{
      ...
      return {title:m.entryId==='second'?'Reference':'Independent studio',description:'Original demonstration. Supplied by an optional plugin, without selecting a song.',
        layout:'cards',append:false,collectionHandle:'collection-'+selected,cards:[{handle:'card-'+selected,title:['Overview content','Journal content','Archive content'][selected],text:'Plain text <script>never executed</script>',actions:[]}],actions:[],
