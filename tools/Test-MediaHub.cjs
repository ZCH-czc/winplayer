    ...
    for (const theme of ['light','dark']) for (const scale of [1,1.5,2]) {
      ...
      await page.exposeFunction('testBridge', async m => {
        ...
        if (m.action === 'requestPlatformExtras') {
          ...
          await page.evaluate(({m,error}) => window.Auralis.setPlatformExtras({...m,error,nextPageHandle:m.kind==='comments'&&!m.pageHandle?'comments-next':null,items:m.kind==='parts' ? [1,2].map(n=>({id:`part${n}`,handle:`part${n}`,kind:'online',providerId:'bili',title:`P${n} · 片段`,artist:'测试',durationSeconds:60,hasMusicVideo:true})) : m.kind==='comments' ? Array.from({length:20},(_,i)=>({handle:'community-'+(i+(m.pageHandle?20:0)).toString(16).padStart(32,'0'),author:`测试作者 ${i}`,text:'<img src=x onerror=alert(1)> 纯文本评论',likeCount:3})) : Array.from({length:50},(_,i)=>({seconds:1+i*.2,text:`测试弹幕 ${i}`}))}), {m,error});
        }
        if (m.action === 'setEmbeddedVideo' && (!holdVideo||!m.enabled)) await page.evaluate(m => window.Auralis.setEmbeddedVideoState({...m,
          qualities:m.enabled?[{handle:'vq-'+ '1'.repeat(32),label:'720p'},{handle:'vq-'+ '2'.repeat(32),label:'1080p'}]:[],
          selectedQualityHandle:m.qualityHandle||'vq-'+ '1'.repeat(32)}),m);
      });
      ...
      await page.waitForSelector('.discussion-list article');
      assert.equal(await page.locator('.discussion-list img').count(),0,'untrusted HTML comment remains literal');
      await page.locator('dialog[aria-labelledby="mediaDialogTitle"]').evaluate(n=>Promise.allSettled(n.getAnimations().map(a=>a.finished)));
      const drawer=await page.locator('dialog[aria-labelledby="mediaDialogTitle"]').boundingBox();
      await checkSurface(page);
      const contextTop=await page.locator('.discussion-subject').evaluate(n=>n.getBoundingClientRect().top);
      ...
      await page.evaluate(()=>{window.firstCommentNode=document.querySelector('.discussion-list article');});
      failNextComments=true;
      await page.locator('.discussion-scroll').evaluate(n=>{n.scrollTop=n.scrollHeight;});
      await page.locator('.discussion-footer').getByRole('button',{name:'重试',exact:true}).waitFor();
      assert.equal(await page.locator('.discussion-subject').evaluate(n=>n.getBoundingClientRect().top),contextTop,'track context remains outside the scroller');
      assert.equal(await page.locator('.discussion-list article').count(),20);
      await page.locator('.discussion-footer button').first().scrollIntoViewIfNeeded();
      const scrollBefore=await page.locator('.discussion-scroll').evaluate(n=>n.scrollTop);
      await page.locator('.discussion-footer button').first().click();
      await page.waitForFunction(()=>document.querySelectorAll('.discussion-list article').length===40);
      const scrollAfter=await page.locator('.discussion-scroll').evaluate(n=>n.scrollTop);
      assert(Math.abs(scrollAfter-scrollBefore)<80,`pagination keeps scroll: ${scrollBefore} → ${scrollAfter}`);
      assert(await page.evaluate(()=>window.firstCommentNode===document.querySelector('.discussion-list article')),'append preserves already-read nodes');
      ...
      await page.evaluate(m=>window.Auralis.setPlatformExtras({...m,items:[{handle:'community-'+'1'.repeat(32),author:'有头像',text:'头像测试',avatarUrl:'https://platform-art.auralis.local/avatar-test'},{handle:'community-'+'2'.repeat(32),author:'缺头像',text:'兜底',avatarUrl:'https://untrusted.invalid/avatar.png'}]}),avatarRequest);
      ...
      await page.evaluate(m=>window.Auralis.setPlatformExtras({...m,items:[{handle:'community-'+'3'.repeat(32),author:'表情测试',text:'你好[打call]<script>不是代码</script>',emotes:[{text:'[打call]',url:'https://platform-art.auralis.local/avatar-test'},{text:'代码',url:'https://evil.invalid/a.png'}]}]}),emoteRequest);
      ...
      await openVideo();
      await page.locator('#videoQuality').waitFor({state:'visible'});
      const beforeQuality=messages.filter(m=>m.action==='setEmbeddedVideo').length;
      await page.locator('#videoQuality').selectOption('vq-'+'2'.repeat(32));
      await until(()=>messages.filter(m=>m.action==='setEmbeddedVideo').length>beforeQuality,'quality request');
      await page.waitForFunction(()=>document.querySelector('#videoQuality').value==='vq-'+'2'.repeat(32)&&!document.querySelector('#videoQuality').disabled);
      assert.equal(messages.findLast(m=>m.action==='setEmbeddedVideo').qualityHandle,'vq-'+'2'.repeat(32));
      assert.equal(await page.locator('#videoQuality').evaluate(n=>n.offsetHeight>=40),true);
      await page.locator('#videoQuality').selectOption('');
      await until(()=>messages.filter(m=>m.action==='setEmbeddedVideo').length>beforeQuality+1,'automatic quality request');
      assert(!messages.findLast(m=>m.action==='setEmbeddedVideo').qualityHandle);
      await page.waitForFunction(()=>document.querySelector('#videoQuality').value===''&&!document.querySelector('#videoQuality').disabled);
      ...
      await page.screenshot({path:path.join(output,`${theme}-${scale}-video-quality.png`)});
      await page.locator('[data-media-action="options"]').click();
      ...
      assert(await page.locator('#embeddedVideoSurface canvas').isVisible(),'shared discussion keeps the video surface mounted');
      assert.equal(await page.locator('#embeddedVideoSurface canvas').evaluate(n=>n.getContext('2d').getImageData(0,0,1,1).data[3]),255,'shared discussion does not clear the decoded frame');
      ...
      await page.waitForFunction(()=>document.querySelector('.discussion-subject').textContent==='P1 · 片段');
      ...
      assert(!(await page.locator('.discussion-list').textContent()).includes('stale comment'));
      ...
      holdComments=false;
      await page.locator('[data-media-action="comments"]').click();
      await page.locator('.discussion-list article').first().waitFor();
