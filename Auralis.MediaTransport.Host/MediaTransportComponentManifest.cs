...
public sealed record MediaTransportComponentFile(string Path, long Length, string Sha256);

/// <summary>Backend-only immutable package declaration, not approval to execute it.</summary>
public sealed class MediaTransportComponentManifest
{
    public const string FileName = "transport.component.json";
    public const int MaximumManifestBytes = 256 * 1024;
    public const int MaximumFiles = 256;
    public const long MaximumPayloadBytes = 64L * 1024 * 1024;
    public MediaTransportDescriptor Descriptor { get; }
    public string RuntimeIdentifier { get; }
    public string EntryAssembly { get; }
    public string EntryType { get; }
    public IReadOnlyList<MediaTransportComponentFile> Files { get; }

    private MediaTransportComponentManifest(MediaTransportDescriptor descriptor, string runtime, string assembly,
        string type, MediaTransportComponentFile[] files)
    ...
    public static MediaTransportComponentManifest Parse(ReadOnlyMemory<byte> utf8)
    {
        if (utf8.Length is 0 or > MaximumManifestBytes) throw new FormatException("Invalid transport manifest.");
        try
        {
            ...
            if (root["schemaVersion"].GetInt32() != 1 || root["kind"].GetString() != "mediaTransport") throw Invalid();
            ...
            var capabilities = MediaTransportCapabilities.None;
            foreach (var value in root["capabilities"].EnumerateArray())
            {
                var flag = String(value, 32) switch
                {
                    "authorizedHttp" => MediaTransportCapabilities.AuthorizedHttp,
                    "completeBuffering" => MediaTransportCapabilities.CompleteBuffering,
                    "prefetch" => MediaTransportCapabilities.Prefetch,
                    "independentResources" => MediaTransportCapabilities.IndependentResources,
                    "sharedBudget" => MediaTransportCapabilities.SharedBudget,
                    "sharedRequests" => MediaTransportCapabilities.SharedRequests, _ => throw Invalid()
                };
                if ((capabilities & flag) != 0) throw Invalid();
                capabilities |= flag;
            }
            if (capabilities == MediaTransportCapabilities.None) throw Invalid();
            var files = new List<MediaTransportComponentFile>();
    ...
    private static FormatException Invalid() => new("Invalid transport manifest.");
}
