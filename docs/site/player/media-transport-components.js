(() => {
  ...
  function render() {
    ...
    host.innerHTML = `<div class="settings-card-header"><div><h2>${text('传输组件')}</h2><p>${text('选择媒体下载与缓存组件；更改在重启后生效。')}</p></div>
      <div class="plugin-row-actions"><button type="button" class="secondary-button" data-tr-action="list" data-tr-focus="refresh" ${disabled(s.pending)}>${text('刷新')}</button>
      <button type="button" class="accent-button" data-tr-action="pick" data-tr-focus="pick" ${disabled(locked || invalid)}>${text('导入传输组件')}</button></div></div>
      <p class="plugin-inventory-status" role="status">${s.pending ? text('正在处理传输组件…') : text('支持 .auralis-transport.zip；导入后默认关闭，不影响当前播放。')}</p>
      ...
      ${data ? `<div class="setting-row"><div class="setting-label"><strong>${text('本次启动传输组件')}</strong><span data-i18n-skip>${escape(data.current?.displayName)} · ${escape(data.current?.version)}</span></div><span class="plugin-state">${text(data.current?.bundled ? '随包默认' : '外置组件')}</span></div>
      ${data.current?.fellBack ? `<p class="plugin-inventory-status" role="status">${text('外置组件不可用，已回退随包默认。')}</p>` : ''}
      <div class="setting-row"><div class="setting-label"><strong>${text('使用随包默认')}</strong><span>${text('保留随包媒体传输，不需要导入组件。')}</span></div><button type="button" class="secondary-button" data-tr-action="default" data-tr-focus="default" ${disabled(locked || invalid || data.selectedId === null)}>${text(data.selectedId === null ? '已选择' : '设为下次启动')}</button></div>
      ${data.items.length ? `<ul class="plugin-inventory-list">${data.items.map(item => `<li class="setting-row plugin-inventory-row"><div class="setting-label"><strong data-i18n-skip>${escape(item.displayName)}</strong><small data-i18n-skip>${escape(item.id)} · ${escape(item.version)}</small>${!item.available ? `<span>${text('文件不可用或组件不兼容')}</span>` : ''}</div><div class="plugin-row-actions"><button type="button" class="secondary-button" data-tr-action="select" data-tr-id="${escape(item.id)}" data-tr-focus="select-${escape(item.id)}" ${disabled(locked || invalid || !item.enabled || !item.available || item.selected)}>${text(item.selected ? '已选择' : '设为下次启动')}</button><button type="button" class="switch ${item.enabled ? 'on' : ''}" role="switch" aria-label="${text('启用传输组件')} ${escape(item.displayName)}" aria-checked="${!!item.enabled}" data-tr-action="enable" data-tr-id="${escape(item.id)}" data-tr-focus="enable-${escape(item.id)}" ${disabled(locked || invalid || (!item.enabled && !item.available))}></button></div></li>`).join('')}</ul>` : `<div class="plugin-inventory-empty"><strong>${text('尚未导入传输组件')}</strong><p>${text('保留随包媒体传输，不需要导入组件。')}</p></div>`}
      ${data.restartRequired ? `<div class="plugin-import-actions"><span>${text('更改已保存，重启后使用新的传输组件。当前播放会停止。')}</span><button type="button" class="accent-button" data-tr-action="restart" ${disabled(locked)}>${text('立即重启')}</button></div>` : ''}` : ''}
      ${preview ? `<section class="plugin-import-review" aria-label="${text('确认导入传输组件')}"><h3>${text('确认导入传输组件')}</h3><p data-i18n-skip>${escape(preview.displayName)} · ${escape(preview.version)}</p><p data-i18n-skip>${escape(preview.id)}</p>
      ...
      <label class="plugin-trust-choice"><input type="checkbox" data-tr-trust ${s.trusted ? 'checked' : ''} ${disabled(s.pending)}><span>${text('我信任此传输组件的来源，了解它可处理媒体地址与请求头，并在播放器进程内运行，不是安全沙箱。')}</span></label>
      <div class="plugin-import-actions"><button type="button" class="secondary-button" data-tr-action="cancel" ${disabled(s.pending)}>${text('取消')}</button><button type="button" class="accent-button" data-tr-action="confirm" ${disabled(s.pending || !s.trusted || !/^[a-f0-9]{32}$/.test(preview.token || ''))}>${text('确认导入')}</button></div></section>` : ''}`;
    if (needsAttention()) env.onAttention?.();
    if (!s.pending && s.focus) {
      [...host.querySelectorAll('[data-tr-focus]')].find(node => node.dataset.trFocus === s.focus)?.focus({preventScroll:true});
      s.focus = '';
    }
  }
  function request(operation, extra = {}) {
    ...
    render();
    try { env.nativePost('manageMediaTransportComponents', { requestId, operation, ...extra }); }
  ...
  function receive(payload) {
    ...
    else if (!payload.error && payload.operation === 'pick' && state.preview) host?.querySelector('[data-tr-trust]')?.focus();
  }
  function leave() {
  ...
  function mount(element, context) {
    ...
    host.onclick = event => {
      const button = event.target.closest('[data-tr-action]');
      if (!button || button.disabled || state.pending) return;
      const action = button.dataset.trAction, id = button.dataset.trId;
      state.focus = button.dataset.trFocus || '';
    ...
    host.onchange = event => {
      if (event.target.matches('[data-tr-trust]')) {
        state.trusted = event.target.checked;
        const confirm = host.querySelector('[data-tr-action="confirm"]');
  ...
  window.AuralisMediaTransportComponents = Object.freeze({ mount, receive, leave, refresh:render, isBusy:busy, needsAttention });
})();
