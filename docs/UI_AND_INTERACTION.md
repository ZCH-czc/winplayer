...
2026-09-23 图片恢复：插件页面、兼容作者页、共享评论与图库只对宿主已批准的
`platform-art.auralis.local` 不透明图片句柄提供显式重试。失败保留头像首字或图片尺寸，
不会自动循环请求；用户触发时用仅含递增 retry 参数的新代理请求避开失败缓存，
不重新读取整页、不重置讨论或播放，也不暴露来源 URL。检索结果的整卡按钮在头像失败时
支持点击头像或按 R 重试，Enter 仍进入作者。图片重试失败继续保留可达入口。

2026-09-23 宽屏目录与搜索偏好：通用作者目录在有效资料头下使用自适应等列卡片，
音乐人作品和专辑不再被 920px 单列约束；不足宽度时自然回到单列。评论抽屉在可用 CSS
宽度达到 1500px 时最多扩展至 700px、占视口不超过 42%，窄窗继续沿用 430px 上限。
这覆盖下文旧版固定 430px 约束，不依赖物理 DPI 比例，不改变评论滚动、焦点、动画或请求。
设置行改称“搜索偏好”（本地优先／在线优先），对应的 DOM、键盘顺序与持久化值不变；
没有已启用在线搜索来源时不显示虚构在线分区。宽屏卡片与评论仍遵循减少动画及高对比度规则。

2026-09-22 动态详情自适应：替代下文历史“详情始终单列”的约束。带有效讨论的详情共用
1440px 页面宽度，可用内容达到980 CSS px且窗口高于600px时，左侧动态、右侧评论；
不足时恢复上下阅读，无评论能力的详情保留920px。断点已包含DPI/浏览器缩放，不乘物理像素比。
进入详情按既有句柄读取一次评论，不抢正文标题焦点/滚动；显式评论入口才聚焦讨论。
评论区宽窗吸顶、内部有界滚动，标题与排序保持可见；切窄窗不重建节点或重读数据。
保留原轻量进入、回复返回和减少动画策略，不动画宽高，不增加模糊或分隔线。
这是通用宿主呈现更新，插件API/SDK/平台权限不变；真实账号与跨屏DPI验收不由合成测试代替。

2026-09-22：投稿等通用插件目录读取按可用 CSS 宽高计算约 2–4 行的批量提示（6–20 条）。
接近底部 0.4 个视口（最多360px）串行自动续页，原节点保留，新增内容复用既有轻量淡入；
错误停止自动读取，仍有键盘可达的加载/重试入口。沿用200张阅读窗硬上限与下一段/返回，不无限堆积DOM。
窗口缩放只影响下一次新查询的页大小，不更改已发放的分页偏移。该批量策略不是原生跨显示器性能验收。
全屏视频头部增加通用画质下拉，40px命中、窄窗换行，只显示插件授权返回的可播放档位，旧插件不出现虚假选项。
切换保留最后一帧；等待/错误沿用原视频反馈，声音由Native管理，不创建第二个播放器。
设置 → 本地音乐库增加“搜索偏好”（本地优先/在线优先）；所有本地分组作为整体前置或后置，
DOM与键盘顺序一致。偏好仅本机持久化，不新增搜索、账号或播放请求。


2026-09-21 导航动效：插件主页面在响应完成且通过原有身份/格式检查后，才执行旧正文 100ms 退场，
新正文 240ms 进入；位移不超过 14px，进入详情与返回方向相反，分区依据左右顺序。
等待网络或失败时不清空/淡化旧内容；退场时仅正文 inert，返回与页签仍可取消，过期提交不生效。
相同作者资料区保留节点；页签下划线以 280ms transform 过渡，换行/滚动位置不一致时直接就位。
缓存返回同样过渡并恢复原滚动/焦点；刷新不重播整页，追加仅沿用可见、至多12项的既有淡入。
评论/楼中楼正文沿用 240ms 方向过渡，标题不移动，快响应接续当前时间轴。
减少动画/隐藏/缩放时结束视觉过程并提交仍有效的数据；离页、撤销、关闭或退出取消尚未提交的页面。
不克隆卡片/图片、不动画尺寸或模糊、不添加全局播放命令；SDK、原生消息和平台实现不变。
motion 合成套件检查两阶段、缓存、快速连点、失败、实时减少动画、能力撤销及播放/预取消息不变；
此结果不是原生窗口流畅度、实际声音或真实平台验收。

2026-09-21 页面构图：详情标题、返回/刷新、来源路径、正文与评论共用 920px 阅读列；
作者资料区不随动态/投稿分区缩放，避免切标签时横向跳动。有媒体的详情在宽窗并排呈现封面和说明，
740px 以下回到上下排列；评论仍在下方。无新增尺寸/模糊动画、不改变平台内容或只读导航授权。
纯媒体目录的标题显示两行，完整标题保留在 tooltip/可访问文本；同排底部操作对齐，命中至少40px。
已有作者/封面时长不重复展示，独立音乐人署名、不同作品标题、试听/不可用状态仍保留。
目录显示短日期，完整时间保存在 datetime、tooltip 和可访问名称；详情保持完整时间，语言切换同步更新。
目录评论入口使用16px线性图标与数量，完整“查看评论”语义保留；不增加平台专属图标或边框。
语言切换就地更新宿主的播放/入队、评论和可用状态，不替换卡片节点、不把插件标题/作者/标签当作翻译 key。
profile 合成回归增加投稿 → 播放/入队 → 详情/评论/回复 → 返回/分区切换，验证不误发播放/预取命令。
这只验证 Web 呈现与消息调度，不代表原生声音连续性、真实平台或跨显示器 DPI 验收。

2026-09-21 共用图片查看器：Pages 与旧 CreatorFeed 使用同一实现，固定标题/操作栏和有界图片视口，
默认适应窗口；用户可切换长图阅读，按视口宽度显示并在正文内滚动。切图保持工具栏 DOM，复位图片滚动，
只请求当前选中的宿主图片代理，不预取相邻图片、不缓存整个图集的解码位图。
加载复用统一圆环；错误或 20 秒超时提供显式重试，无自动重试。切图/关闭/撤销时废弃旧回调并释放图片。
左右键切图，Esc 关闭，Tab 可进入长图滚动区域；关闭恢复原缩略图焦点，不滚动底层阅读页。
进入/退出 220/140ms，图片成功后仅 160ms 淡入；减少动画和隐藏文档跳过动画，不动画图片尺寸或模糊。
没有新的 Native 消息、SDK 能力或平台数据读取权限，图像始终经已批准的宿主代理。

2026-09-21 动态阅读节奏：正文与作者、标题、引用、操作区使用 8/12/16px 分层间距，
feed/detail 的正文 14px、1.75 行高；窄窗卡片 16px、引用 12px 内边距，不添加卡片模糊或分隔线。
两图和四图使用两列，其余多图保留三列；单图在固定 16:9 占位内 contain 完整展示，点击图库看大图。
图像解码不改变占位尺寸；移除旧固定图片高度造成的溢出裁切。焦点轮廓位于缩略图内侧。
超过 280 字符或七行的非详情正文检查实际溢出，仅截断时提供展开/收起，aria-controls 指向唯一正文；详情完整展示。
收起后确保按钮仍在视口内，保留焦点，不动画高度、不平滑滚动；已有追加/返回缓存规则不变。
所有操作仍保留 40px 命中，头像首字作为作者数据跳过 UI 翻译。不改 SDK、平台请求或账号权限。

2026-09-21 作者资料层次：通用作者入口（或带分区与简介图片的主页面）将唯一标题、头像和简介整合到资料区，
返回/刷新保持独立 40px 操作行。图片失效保持首字头像与尺寸，不追加背景图片请求，不推测粉丝/播放等统计。
仅资料区右侧 320×140px 的静态色晕使用 24px blur；正文、头像、封面不模糊，各卡片不加 backdrop-filter。
资料页吸顶分区栏支持 10px 背景模糊与 94% 实色底，减少透明度/强制颜色时回退实色；不支持时默认实色。
这是对 2026-09-19 分区实体背景规则的有界修订。无滚动监听、无限动画、平台 ID 分支或新 SDK 字段。

2026-09-20 作者页视觉修订：主页简介、分区和查询表单不再嵌套描边面板；动态以留白和表面色分隔。
纯媒体目录使用等列封面网格而非瀑布流，封面、标题、作者与日期依次排列，保留小字号、40px 操作命中区。
插件声明 Open 时封面与标题进入只读详情；无 Open 的旧媒体卡片点击封面/标题是明确播放操作，受可播放状态约束。
图集与媒体封面读取失败保留尺寸和重试说明；显式重试不触发播放，不自动循环联网。
投稿详情使用已有 Pages v8，平台接口仍留在插件，未增加公开核心平台分支。

2026-09-20 社区阅读连续性：独立动态详情的返回/标题/刷新栏在主内容滚动中吸顶，使用实体主题背景。
页面导航、刷新和返回失败的提示在内容视口底部可见，保留原文并提供明确重试，不盖住播放栏；
分页加载/失败仍在列表末尾，不作为导航浮条弹出。无操作时不保留空浮层。
作者搜索与页面状态区在按钮替换为等待文案时接住键盘焦点，失败后返回重试按钮，不落到页面 body。
新增 journey 合成回归覆盖用户分类中非预览作者 → Pages v8 作者主页 → 动态 → 评论/楼中楼 → 原搜索结果，
核对分类、查询、缓存、滚动、入口焦点和无播放副作用。此项不是原生窗口或真实账号验收。

2026-09-20 阅读布局调整：有 CreatorSearch 能力的来源在总搜索同一区域提供综合、音视频、用户分类，
复用同一个查询和两类结果缓存；分类切换只过滤呈现，不重新查询，也不能把用户句柄当作曲目。
未声明能力时不出现用户分类；来源/查询/修订改变仍按原规则取消旧请求。
带详情导航的动态卡片评论进入独立主页面，正文在上、评论在下；旧版无详情声明的页面保留兼容抽屉。
全屏播放中的评论仍使用右侧抽屉，不改变播放。动态用间隔圆角卡片，转发原文使用无边框浅色内嵌区，
四图为二乘二布局；评论用留白分组，日期、计数和回复入口在同一紧凑行，视觉按钮缩小而命中区仍至少40px。
此轮仅调整宿主通用呈现，不新增平台专属协议或账号写操作；Pages v8 插件无需因此重新编译。

2026-09-20：详情内评论和全屏／列表右侧评论抽屉统一使用 page-discussion。
保留抽屉、关闭按钮和原进退动画；评论与楼中楼使用同一套独立阅读视图，不再向主评论下堆叠回复卡片。
楼中楼 Esc 先返回主评论，恢复排序、滚动与入口焦点；再次 Esc 关闭抽屉并回到触发按钮。
标题／主题／排序固定，正文只有一个滚动区域；继续使用主题 token、40px 命中及 reduced-motion 路径。
分页保留已读节点，有界分段并提供“继续阅读／上一段”；错误停止自动读取，关闭即撤销等待。
全屏评论跟随当前曲目，列表评论固定到所选主题；修订变化关闭旧讨论，仅移除回复能力时保留主评论。
评论导航不发送播放／暂停／视频切换命令、不清空 canvas，不影响音频或视频当前会话。
本次仅通用宿主 Web 资源调整，不改变插件契约。真实账号、Native 播放及跨屏 DPI 仍需独立验收。

2026-09-20：全局插件页面的讨论按钮补齐通用 online 评论上下文，使用 Native 发出的 discussionHandle。
不要求全局页面伪造当前歌曲；仍检查 Comments 能力与后端归属，不启动播放、不改变队列。
修复全局入口能显示评论按钮却不发起读取的问题，不增加平台分支或 SDK 特性。

2026-09-19 插件页面导航连续性：标题下显示来源路径；返回键的可访问名称包含上一页标题。
主页面分区标签在正文滚动中吸顶，不要求先滚回顶部才能切换分区。保持普通主题实体背景，不添加滚动模糊。
返回/分区回切仍经 Native 读取校验；成功后可恢复当前会话 60 秒内的阅读节点、滚动、展开状态和焦点。
最多保留 6 个视图、每视图不超过 100 张卡片；超限或过期重新渲染，不扩大平台数据/分页授权。
快照绑定生成时的界面语言，切换语言后不恢复旧语言控件。
显式刷新绕过视图恢复；失败保留当前内容。离页、关闭、账号/设置/插件修订变化释放快照；不写磁盘。
该通用宿主呈现改动没有平台 ID 分支，不改变插件 SDK/消息契约。

2026-09-19 通用作者搜索与插件社区页面细化（契约版本不变）：
作者结果先于在线曲目结果展示；翻页原位追加，保留已有头像、焦点和卡片，不把作者当作音轨。
主页面标题旁提供刷新与读取状态，读取/失败保留旧内容；长文提供显式展开/收起和 aria-expanded。
页面、作者卡片、评论和楼中楼复用 page-motion.js：仅可视新增节点（最多 12 个）进行 300ms 淡入与 8px 位移，
交错延迟上限 192ms（2026-09-21 细化）；减少动态效果、隐藏文档跳过，隐藏时取消在途动画，不添加无限循环或布局动画。
卡片继续沿用主题 surface/stroke、键盘焦点与 40px 操作区域，宽窗双列、窄窗单列；图库与评论维持独立层级。
这是宿主通用渲染能力的细化，需要更新宿主 Web 资源；不是允许插件注入样式或脚本。

2026-09-13 第十三阶段：平台管理列表提供“选择旧版包”，只打开原生单包选择器，不直接切换旧目录。
预览在原有页内确认区展示当前版本、候选版本、SDK 和分类声明增删；访问变化默认展开，内容转义、长标识断行。
使用既有 surface/stroke/radius、40px 控件，宽窄窗口操作组换行，正文仍限高滚动，确认/取消保持独立可达。
未预选信任、未乐观启用；恢复明确说明不自动恢复账号/设置。无关清单刷新不重建同一审阅，不丢展开与勾选。
候选/操作状态改变必须重新批准。取消/选择失败回到原插件入口；迟到回调不能重开预览。
继承现有设置页动效与减少动画，无新的循环动画。旧包内容摘要与原 ZIP SHA-256 使用不同标签。

2026-09-13 Pages v6 媒体卡片沿用 surface/stroke/radius 和 40px 显式按钮。
相同卡片标题不重复显示，作者/时长作为次级信息；封面不与已有图库重复堆叠。
播放与加入队列独立于只读导航；不可用项禁用且显示原因，试听限制保留为文字。
控件可用键盘操作；窄窗口换行、长文断行，深浅色和减少动态效果复用既有路径，没有新增强制动画。
播放失败保留页面/当前队列，加入队列不打断音频；全屏播放器沿用同一播放状态。
测试为合成桥接的 100/150/200% 浏览器缩放与 restored/maximized/fullscreen UI 状态，不等于真实跨屏 DPI。

2026-09-13 Pages v5 复用既有 Fluent 主页面，不新增弹窗或插件 CSS。
目标读取成功才替换内容，失败保留来源；返回重读来源后恢复已提交查询、滚动位置与可识别的按钮焦点，
无法匹配才聚焦标题。Esc 仍先关闭图库/讨论，再返回页面。侧栏来源入口保持选中，底栏播放器不变。
原有 40px 控件、宽窄排布、深浅色与 reduced-motion 路径保持。类型化实体只在 Native 管理，不显示原始 ID。

2026-09-13 Settings v2：插件声明分组标题、说明和中英文文本；无插件无分组。
宿主复用 settings-card/setting-row/stroke/radius，不接受插件样式。组名只显示一次，
行内不重复平台名；控件至少 40 CSS px，窄窗口向下排列，提示文字保持可读。
依赖未满足时保持值但禁用控件；不以降低整行透明度遮蔽说明。界面不得在父设置回执前启用子设置。
条件/声明变化废弃旧草稿和晚到回执；保存失败保留可重试草稿。
仅文本刷新不得撤销有效保存。没有新增动画，继承原设置页面的键盘与 reduced-motion 路径。
详见 PLUGIN_SETTINGS.md；UI 测试为隔离 Edge 合成桥接，不代表原生窗口 DPI 验收。

2026-09-13 第四阶段：Pages v4 查询区沿用 surface/stroke/radius、setting-text-input 和 accent-button，
文本与选项标签对齐、40px 命中；窄窗口换行/提交按钮整行，选择项用原生 radio 语义与主题选中边框。
控件不持久化输入；输入/选项变化不查询，中文输入法确认不误提交，Enter/按钮才提交。
编辑时取消旧请求并停止旧结果自动追加；新请求失败保留草稿和已读结果，重试/返回保留提交的条件。
查询完成不抢走输入焦点；键盘输入不能触发播放器空格快捷键，减少动画沿用宿主页面规则。

2026-09-13 第三阶段：插件全局入口位于可选侧栏组，只显示当前声明且配置可用的 GlobalPages。
沿用 nav-item/Fluent 线性图标、选中指示和40px命中；内容独立限高滚动，不挤掉设置与播放栏。
Pages v3 标签使用 tablist/tab/tabpanel、选中下划线；方向/Home/End 只移焦点，Enter/Space 读取。
读取中可切换另一标签以取消旧请求，失败不改变选中或正文；成功后正文160ms轻位移/淡入，
减少动画立即呈现。返回恢复侧栏或内容来源焦点，停用/修订失效立即关闭并丢弃旧回调。

2026-09-13 Pages 第二阶段：插件声明 presentation:page 后使用通用主内容 route plugin，
返回键与标题左对齐，侧栏和播放栏保留，宽窗双列/窄窗单列。dialog 为旧版默认且保持兼容。
增量列表原位追加/去重，不重建已读卡片、不抢焦点和滚动；失败停止自动翻页，保留文字和重试。
图片组使用独立图库（左右键、Esc、返回缩略图焦点）；Discussion 使用既有右侧评论/楼中楼和日期。
新图库与页面均使用主题 token、40px 命中、单次轻量过渡及减少动画路径；不是平台注入 HTML/CSS。
通用评论窗口的正文查询必须限定其 dialog，不能使用全局 .media-dialog-body 误写主页面。

2026-09-13 社区主页面：作者动态改为主内容区完整 route，不再使用模态弹窗或遮罩；侧栏和底部播放栏保留。
宽窗双列、窄窗单列卡片；卡片包含头像、作者、日期、段落和图片组。大图查看器仍是可关闭的临时层，支持逐张按钮。
评论仍从右侧展开，关闭后保留动态滚动位置；Escape 先关评论/图片，再返回来源页面。
作者搜索按 CreatorSearch 能力在当前来源下显示独立结果，自动防抖、串行分页，与曲目和队列分离。
返回恢复原搜索框展开状态、关键词、结果和可见入口焦点；加载/错误/无结果有文字，减少动画沿用 route 规则。
以下 2026-09-12 弹层记录为历史行为，现已由本条替代；仅资料的声明式 Pages 路径仍可使用紧凑弹层。

2026-09-13：声明式插件页面共用 Fluent 弹层，核心渲染卡片/列表和日期、返回/重试；内容由插件提供。
窄窗单列，正文独立滚动，多入口收进一个选择器，避免挤满播放工具栏。所有动画有减少动画路径。
契约和尚未迁移的范围见 [PLUGIN_PAGES.md](PLUGIN_PAGES.md)。

仅 CreatorProfile 时使用紧凑资料弹层，不显示动态标题、空态或加载更多；长简介保留段落并在正文区滚动。

2026-09-12：可选作者主页使用共享 Fluent 弹层，动态为双列自适应卡片、窄窗单列，正文独立滚动。
评论仍为右侧抽屉，可叠在作者页上并逐层 Escape 返回；展示日期、排序、楼中楼分页。
搜索的窄窗提供紧凑作者入口，全屏作者名也支持键盘。能力缺失/撤销时隐藏并关闭对应表面，不打断播放。
细节与未覆盖的原生 DPI / 真实网络验收见 [CREATOR_COMMUNITY.md](CREATOR_COMMUNITY.md)。
...
Pages v7 阅读组件（开发阶段）：动态 `feed` 保留时间和 DOM 顺序，可用内容宽度达到 980 CSS px 时双列，
否则单列；不依据物理像素或 DPR 强制挤出双列。作品目录仍使用原网格。
详情正文默认完整展开；评论是详情内的从属区域，各窗口尺寸均置于正文下方，
不是新的全屏遮罩弹窗。楼中楼显示根评论和独立回复列表，返回保留排序、位置与焦点。
子视图导航使用 160 ms / 4 px 的小幅淡入，减少动画时立即切换；追加评论不重播整页动画。
遵守现有主题 token、40 px 按钮命中区和键盘焦点。语言切换仅更新本地呈现，不重新请求评论。
自动追加有数量上限，后续段落显式继续；失败不自动循环请求。
这一阶段未替换全屏播放器原评论抽屉；真实插件和原生 DPI 验收单独记录。

Pages v8 阅读组件：转发原文缩进到同一阅读单元，使用主题 surface/stroke/radius token，
不再与分享者并列成两张独立卡片。作者姓名提供显式、可键盘操作的主页入口。
主动态与引用评论共享详情内讨论区，但显示所选作者/主题，分别使用 Native 签发的句柄。
正文片段只允许文字与批准的行内图片；失败/减少动画保留文字。不可用原文无伪造操作。
主内容、引用、原作者来回导航仍用统一历史/焦点，引用不递归堆叠；目录网格保持不变。
...
- `loading-state.js/css` 仅负责宿主呈现，不引入平台分支、请求或 SDK 权限。插件页面、作者搜索、兼容动态页和共享评论使用同一状态组件。
- 首次读取显示中性三行占位；追加只在列表末尾提示；刷新保留当前内容，成功后保留阅读位置，不重复播放整页入场。错误停止加载装饰并留下重试入口。
- 文字立即显示，装饰延迟 160ms 后淡入，不人为延长请求。圆环统一 1400ms 连续转动；搜索、音频准备、视频等待、扫描和歌词等待沿用各自尺寸但共享节奏。歌词来源细线只改变透明度。
- 减少动画时显示静态状态；文档隐藏时暂停加载动画。不使用假百分比、大面积闪烁或逐帧布局计算。
- `npm run test:ui -- loading` 使用隔离合成数据验证初次/追加/刷新/失败/取消、迟到响应、深浅色与 1/1.5/2 倍浏览器缩放；不是原生播放或真实平台验收。

### 增量阅读与轻量分页

- 作者搜索、插件页面和讨论共用 `AuralisLoading.continuation`。同一 44px 命中区在加载、重试之间保留 DOM 和键盘焦点；请求中仅禁用操作，不销毁按钮。结束时焦点落在状态区域。
- 增量加载只让新出现的可见卡片淡入，最多 12 张、300ms、错峰上限 192ms；旧正文、头像和滚动位置不重建。状态文字淡变 180ms；图片加载完成后淡入 240ms。隐藏页面/减少动画遵循共用 motion 策略。
- 常态作者卡片和分页不使用可见描边；用间距、surface 和悬停轻阴影区分层级，键盘焦点和 Windows 高对比度仍有明确轮廓。
- 同一搜索词、来源和语言下，音视频分页保留作者结果子树；切换分类复用已有数据。分页载入成功才进入新结果，失败保留上一页，不把失败显示成空列表。
- 平台头像的 CDN 尺寸策略属于插件，不进入通用 Web 组件；宿主图片下载上限和域名授权保持不变。

### 通用头像筛选与新内容提示（Pages v9）

- 头像横栏只横向滚动，不挤宽主页面。圆形图片 48px，文字最多两行，选中使用主题色柔和光环；
  常态不堆叠卡片边框。方向键、Home/End、Enter 与可见焦点保留，图片失败回退名称。
- 筛选沿用同一套退场/入场、旧请求取消、缓存视图和返回焦点；切换前后保留横栏位置。
  追加只渐显新增卡片，不重播整页动画。减少动画和隐藏页面遵循全局策略。
- 更新提示放在零高度 sticky 容器内，不改变正文高度或当前滚动位置。按钮有 44px 命中区，
  使用主题 surface 与轻阴影，不让通知本身成为大面积弹窗。
- 检查频率由插件在 60–900 秒范围声明，只在可见在线页面空闲时运行；有新内容只提示，
  用户点击才更新。打开弹窗/全屏播放器或正在导航时推迟；失败停止并显示手动重试。
- `npm run test:ui -- updates` 检查横向溢出、键盘、内容/滚动保留、错误停止、迟到响应、
  隐藏/撤销停止和六组主题/比例。浏览器比例测试不能代替原生窗口的物理 DPI 验收。

### 宽屏阅读与侧栏反馈（2026-09-21）

- Feed 与作者页头同宽，不再嵌套 840px 的居中内容条。双列按下述 2026-09-22 修订接续较短列，保持原数据/键盘顺序；
  详情和评论仍单列。浏览器缩放和窗口变窄时使用容器宽度退回单列，不依赖屏幕分辨率猜测 DPI。
- 插件清单尚未返回时显示通用读取圆环和中性占位，不虚构平台。返回空清单则整个插件区消失。
  已进入插件页面的前台读取在侧栏入口显示同节奏圆环，完成/失败/取消撤下；后台更新检查不闪动入口。
- 插件列表不占用剩余弹性高度；设置与其相距 12px。长插件列表仍有自己的有界滚动区，极矮窗口侧栏也可滚动。
- 综合搜索只有成功返回零作者时移除作者整行；作者分类保留明确空结果，读取中或错误仍有反馈/重试。
- 这些是通用宿主呈现与图片授权修正，不改变平台插件、SDK 版本或增加账号写权限。旧本体需更新才能获得修正。

### 双列交错阅读（2026-09-22）

- 动态宽窗使用交错排布：按数据顺序，将下一张卡片放到当前较短列下方，保留 16px 阅读间距，
  不再等待同行最高卡片。只改变网格位置，不拆分/复制/重排 DOM，不以 CSS columns 改变阅读顺序。
- 展开/收起、图片回退和宽度变化通过有界 ResizeObserver 合并到下一帧排布；使用不受进入动画 transform
  影响的布局高度，避免观察器反馈抖动。追加保留既有节点，仍只对可视新增项做既有淡入。
- 内容容器不足 980 CSS px 时清除双列定位，恢复单列；作品目录继续等列网格、详情继续单列。
  隐藏/离页断开观察器，可见后重新计算；减少动画不改变排布与键盘可用性。无新平台权限或 SDK 契约。
